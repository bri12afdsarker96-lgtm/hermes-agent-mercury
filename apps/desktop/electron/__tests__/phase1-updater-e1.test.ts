...
describe('updater-e1 runtime composition', () => {
  ...
  it('B9: restart-install before downloaded is rejected', async () => {
    h.runtime.initialize()
    ...
    expect(env.errorCode).toBe('minimum-version') // policy-not-established fails here
    expect(h.fake.quitAndInstallCalls).toBe(0)
  })

  // B10 — restart-install on beta/internal in V1 → rejected
  it('B10: restart-install on beta channel is rejected', async () => {
    const h2 = makeHarness({ channel: 'beta' })
    const initResult = h2.runtime.initialize()
    // init accepts the runtime but does NOT wire AppUpdater for beta channel
    // (REMEDIATION-01 §14). Any subsequent restart-install call must be
    // rejected before touching the underlying install seam.
    expect(initResult.kind).toBe('initialized')
    await expect(
      h2.runtime.requestRestartInstall({
        userConfirmed: true,
        safeStoragePreserved: true,
        hasPendingMutations: false,
      }),
    ).rejects.toThrow(/AppUpdater not wired/)
