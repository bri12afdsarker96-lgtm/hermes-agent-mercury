...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'hermes_Agent'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? '正在连接 Hermes 企业服务…' : '登录企业服务',
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Hermes 企业服务会话已失效，请重新登录。') as any
    err.needsCloudLogin = true
    throw err
  }
  ...
  if (authMode === 'oauth') {
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        '刷新 WebSocket 访问凭据时无法连接远程企业服务，请检查网络后重试。'
      )
    }
