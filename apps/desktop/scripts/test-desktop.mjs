...
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'HermesEnterpriseAssistant.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'HermesEnterpriseAssistant'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'HermesEnterpriseAssistant.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'HermesEnterpriseAssistant'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Hermes-企业助手-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Hermes-企业助手-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `hermes_Agent-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}
