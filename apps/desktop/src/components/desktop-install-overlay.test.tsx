...
describe('DesktopInstallOverlay first-run setup', () => {
  it('shows the remote/local choice without installer progress', async () => {
    ...
    expect(await screen.findByText('Set up hermes_Agent')).toBeTruthy()
  ...
  it('continues local bootstrap only when Install Hermes locally is selected', async () => {
    ...
    expect(screen.getByText('Set up hermes_Agent')).toBeTruthy()

    act(() => {
      desktop.emitBootstrapEvent({ type: 'manifest', protocolVersion: 1, stages: [] })
    })

    await waitFor(() => expect(screen.queryByText('Set up hermes_Agent')).toBeNull())
  ...
  it('surfaces a recoverable error when the local-bootstrap bridge is unavailable', async () => {
    ...
    expect(
      await screen.findByText('Local installation could not start. Restart hermes_Agent and try again.')
    ).toBeTruthy()
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    expect(screen.queryByText('Local installation could not start. Restart hermes_Agent and try again.')).toBeTruthy()
  })
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    expect(screen.queryByText('Local installation could not start. Restart hermes_Agent and try again.')).toBeNull()
  })
