...
import { deriveCommonViewModel, viewerHoldsViaCommon } from './view-model'
...
describe('deriveCommonViewModel', () => {
  ...
  it('returns a coherent VM when whoami is null (session still loading)', () => {
    ...
    expect(vm.canWrite).toBe(false)
    expect(vm.capabilityStatus).toBe(null) // no product_capabilities without whoami
  ...
  it('exposes isPartial / isBlocked for the corresponding page.status', () => {
    ...
    expect(
      deriveCommonViewModel({ page: auditPage, whoami: whoamiSuper }).isBlocked
    ).toBe(false) // audit is 'ready' actually, not 'blocked'. Pick another.
  })
})

describe('viewerHoldsViaCommon (re-exported for tests)', () => {
  it('super user with "*" matches any permission', () => {
    expect(viewerHoldsViaCommon(whoamiSuper, 'anything.at.all')).toBe(true)
  })

  it('operator without the perm is denied', () => {
    expect(viewerHoldsViaCommon(whoamiOperator, 'principal.crud')).toBe(false)
  })

  it('operator with the perm is allowed', () => {
    expect(viewerHoldsViaCommon(whoamiOperator, 'followup.read')).toBe(true)
  })

  it('operator with a wildcard prefix is allowed for matching perms', () => {
    const whoamiWithWildcard: Whoami = {
      ...whoamiOperator,
      effective_permissions: ['kb.*', 'followup.read'],
    }
    expect(viewerHoldsViaCommon(whoamiWithWildcard, 'kb.author')).toBe(true)
    expect(viewerHoldsViaCommon(whoamiWithWildcard, 'kb.publish')).toBe(true)
    expect(viewerHoldsViaCommon(whoamiWithWildcard, 'chat.write')).toBe(false)
  })

  it('null whoami denies every perm (unknown viewer = no read)', () => {
    expect(viewerHoldsViaCommon(null, '*')).toBe(false)
  })
})
