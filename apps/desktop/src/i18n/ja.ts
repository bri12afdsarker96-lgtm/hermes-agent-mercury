...
export const ja = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      startingHermesDesktop: 'hermes_Agent を起動中…'
    },
    errors: {
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。hermes_Agent を再起動してください。',
  ...
  settings: {
    ...
    about: {
      ...
      attribution: 'Nous Research の Hermes Agent を基盤にしています · MIT License',
    ...
    gateway: {
      ...
      intro:
        'hermes_Agent はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Hermes バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。ゲートウェイ接続はマシン単位の設定で、プロファイルは接続したゲートウェイから検出されます。',
      ...
      restartingMessage: 'hermes_Agent は保存された設定を使用して再接続します。',
      ...
      sshErrPlatform:
        'サポートされていないリモートプラットフォームです。hermes_Agent の SSH モードは Linux、macOS、Windows のリモートホストに対応しています。',
  ...
  install: {
    ...
    setupChoiceTitle: 'hermes_Agent をセットアップ',
    setupChoiceDesc:
    ...
    settingUpTitle: 'hermes_Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
  ...
  onboarding: {
    headerTitle: 'hermes_Agent のセットアップをしましょう',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには hermes_Agent を再起動してください。',
    restartToSaveImages: '画像を保存するには hermes_Agent を再起動してください',
