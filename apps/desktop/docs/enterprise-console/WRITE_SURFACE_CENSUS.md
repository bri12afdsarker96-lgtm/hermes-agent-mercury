...
| **Enterprise Knowledge** | READY | READY | POST kb-gap-author/reject, knowledge-upload (multipart), -preview(GET), -commit (publish, SYNC), -rollback, -delete (withdraw), -committed(GET sources) | server; commit synchronous+idempotent; DEV capability | full surface built (B14): sources/upload/preview/publish/withdraw/rollback + review |
